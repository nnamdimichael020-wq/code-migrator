...
export function relatedPages(slug) {
  const page = getPair(slug);
  if (!page) return [];
  return getAllPages().filter(
    (p) => p.slug !== slug && p.source === page.source && p.target === page.target
  );
}

/**
 * Dialect-specific review notes for a given source/target combination.
 * Always the full parent-pair list — modifier pages are a shorter cut of
 * the same pair, not a different dialect.
 */
