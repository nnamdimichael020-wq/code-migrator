...
test("name and displayName are accepted and sanitized", () => {
  const { value } = validateReview({ stars: 4, message: "good", name: "  Ada  " });
  assert.equal(value.name, "Ada");
  assert.equal(value.displayName, "Ada");
  const fromDisplayName = validateReview({ stars: 5, message: "great", displayName: "<b>Ada</b>" });
  assert.equal(fromDisplayName.value.displayName, "Ada");
