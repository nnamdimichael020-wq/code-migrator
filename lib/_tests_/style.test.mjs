...
test("schema + SQL conversion treats the DDL as authoritative ground truth", () => {
  ...
  assert.ok(out.includes("the DDL wins"), "conflicts resolve in favour of the DDL");
});

test("Oracle→PostgreSQL carries a concrete type reference table", () => {
  const out = buildInstruction("idiomatic", ORACLE_PG);
  assert.ok(out.includes("Oracle → PostgreSQL type reference"), "the table is injected for this pair");
  assert.ok(out.includes("NUMBER(p, s)"), "precision/scale row present");
  assert.ok(out.includes("NUMERIC(p, s)"), "maps to NUMERIC(p, s)");
  assert.ok(out.includes("VARCHAR2(n)"), "maps sized strings");
  assert.ok(out.includes("keep precision p and scale s exactly"), "precision/scale never dropped");
  assert.ok(out.includes("never INTEGER, never a float"), "unconstrained NUMBER → NUMERIC, not a float");
});

test("schema rules use precision, scale and nullability — never guess", () => {
  const out = buildInstruction("idiomatic", SQL_SCHEMA);
  assert.ok(out.includes("Carry precision and scale through verbatim"), "precision loss is forbidden");
  assert.ok(out.includes("NUMBER(10,2)"), "concrete precision example");
  assert.ok(out.includes("A NOT NULL column must never receive NULL"), "nullability is carried over");
  assert.ok(out.includes("resolve EVERY column reference"), "no invented columns");
});

test("schema rules add schema-aware function rewriting guidance", () => {
  const out = buildInstruction("idiomatic", SQL_SCHEMA);
  assert.ok(out.includes("DATE columns prefer CURRENT_DATE"), "DATE columns pick the right clock");
  assert.ok(out.includes("CURRENT_TIMESTAMP / NOW()"), "time-aware columns use a clock");
  assert.ok(out.includes("NVL / NVL2 / DECODE"), "Oracle conditionals are expanded");
  assert.ok(out.includes("Remove redundant casts"), "no redundant casts");
});

test("pitfalls are proven or dropped against the schema, critical ones survive", () => {
  const out = buildInstruction("idiomatic", SQL_SCHEMA);
  assert.ok(out.includes("prove or drop every warning"), "schema-proven warnings get dropped");
  assert.ok(out.includes("emit it ONLY if it"), "warnings must still be possible in this conversion");
  ...
  assert.ok(out.includes("time-zone warning"), "timezone pitfall is named");
});
...
test("convertedCode must contain only SQL when a schema is supplied", () => {
  ...
  assert.ok(out.includes("aliases preserved"));
});
...
test("schema rules stay out of non-SQL and schema-less conversions", () => {
  for (const out of [
    ...
    assert.ok(!out.includes("Oracle → PostgreSQL type reference"), "type table leaked");
